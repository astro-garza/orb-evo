...
time_f = 1.1e6*torb    # integration time, 1.1e6*torb for PCb simulation
...
for i, time in enumerate(times):
    ...
    tst = str(sim.t)
    sim.move_to_com()
    #f = open('PCb_esim.txt','a',0)
    print "#######################################################"
    print "### After "+str(sim.t)+" years, "+str(sim.N)+" particles remain. ###"
    print "#######################################################"
    #f.write('\n' + '\n' + "After "+str(sim.t)+" years, "+str(sim.N)+" particles remain.")
    if sim.N < 21:
